    ...
    <div className="space-y-6">
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filteredEvents.map((evt) => {
            ...
            return (
              <Card key={evt.id} className="p-5 flex flex-col justify-between hover:border-indigo-300 dark:hover:border-indigo-700 transition-all">
                <div>
                  ...
                  <div className="mt-4 space-y-1.5 text-xs text-slate-600 dark:text-slate-400">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span className="font-medium text-slate-700 dark:text-slate-300">{evt.venue}</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <Users className="w-3.5 h-3.5 text-slate-400 dark:text-slate-500" />
                      <span>Organized by <strong className="text-slate-800 dark:text-slate-200">{evt.organizer}</strong></span>
                    </div>
                  </div>
      ...
      >
        <form onSubmit={handleSubmitRegistration} className="space-y-4">
          ...
          {registerError && (
            <div className="p-3 bg-red-50 dark:bg-red-950/30 border border-red-200 dark:border-red-900/60 rounded-xl flex items-start gap-2.5 text-xs text-red-800 dark:text-red-300 leading-snug">
      ...
      >
        {loadingAttendees ? (
          <div className="py-8 text-center text-xs text-slate-500 dark:text-slate-400">Loading attendees...</div>
        ) : attendees.length === 0 ? (
          ...
          <div className="divide-y divide-slate-100 dark:divide-slate-800 max-h-96 overflow-y-auto">
            {attendees.map((att) => (
              ...
              >
                <div>
                  <strong className="text-slate-800 dark:text-slate-200 font-semibold block">{att.name}</strong>
                  <span className="font-mono text-slate-500 dark:text-slate-400 text-[11px]">{att.student_id}</span>
                </div>
                <Button
                  size="sm"
                  variant="outline"
